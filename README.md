...
`index.html` öffnen – sie leitet auf den Generator weiter. Direkt geht auch
`Vorlagengenerator mh anwaelte v2.dc.html`. Kein Server, kein Build, keine
Installation. Die Dateien laden `support.js` und die Bilder aus `assets/` über
relative Pfade – der Ordner muss also vollständig bleiben.

Über GitHub Pages (Settings → Pages → Branch `main`) ist der Generator unter der
Pages-Adresse direkt aufrufbar.
...
```
index.html                                 Einstieg, leitet auf den Generator weiter
